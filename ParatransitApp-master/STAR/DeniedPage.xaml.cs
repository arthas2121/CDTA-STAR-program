...
namespace STAR
{
    ...
    public partial class DeniedPage : ContentPage
    {
        ...
        async void OnReClicked(object sender, EventArgs e)
        {
            ...
            bool Repeat = await DisplayAlert("SUCCESS", "You want to repeat that trip?", "Yes", "No");

            if (Repeat)
            {
                await Navigation.PushAsync(new RepeatTripPage(TPID));
            }
        }
    }
}