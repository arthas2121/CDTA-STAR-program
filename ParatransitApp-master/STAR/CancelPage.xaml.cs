...
namespace STAR
{
    ...
    public partial class CancelPage : ContentPage
    {
        ...
        public string SID = Application.Current.Properties["SID"] as string;
        public CancelPage()
        {
            InitializeComponent();
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        {
                            if (confirm.Equals("" + reader1.Trim(' '), StringComparison.InvariantCultureIgnoreCase) == true)
                            {
        ...
        async void OnDelete(object sender, EventArgs e)
        {
            ...
            bool answer = await DisplayAlert("Verify Action", "Delete Trip: " + mi.CommandParameter + "?", "DELETE", "CANCEL");
            if (answer)
            {
                int index = Tripinfo.IndexOf(mi.CommandParameter);
                string url = "?STARID=" + SID + "&TripID=" + mi.CommandParameter + "&pua=" + Tripinfo[index].Pickupaddress + "&RAddress=" + Tripinfo[index].PickupTime + "&dest=" + Tripinfo[index].Destination;
                var responseString = HTTPClientInstance.client.GetAsync("https://paratransitservices.azurewebsites.net/canceltrip" + url);
